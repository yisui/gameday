...
the date should be in the right date format, i.e. "yyyy-mm-dd".
...
This function gives the option to display the final scores of all the games on a 
specified date as a `data.frame`. 

The function only takes one argument `date`. Again, the date should be in the right date 
format, i.e. "yyyy-mm-dd". For example, we can check the final scores for the games
...
We can see that it returns a `data.frame` with four columns. The first column shows the 
name of the home team, the second column for the name of the away team. The third column 
gives the score for the home team, while the score for the away team is given on the 
forth column.

Since we can use the function `score(date)` to get a very informative `data.frame` with 
all the final scores for the games at a perticular date, we can go from this 
